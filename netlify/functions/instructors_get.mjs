...
export default async (req) => {
  try {
    ...
    const raw = await store.get(`instructors/${auth.centerId}`);
    if (!raw) return json(200, { ok: true, centerId: auth.centerId, instructors: {}, updatedAt: 0 });

    const rec = JSON.parse(raw);
    const instructors = rec && rec.body ? JSON.parse(rec.body) : {};

    return json(200, { ok: true, centerId: auth.centerId, instructors, updatedAt: rec.updatedAt || 0 });
